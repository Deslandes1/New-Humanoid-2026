    ...
    <body>
        ...
        function update(dt) {{
            ...
            if (state.cmd === 'run') {{
                // Robot is running
                if (state.soccerMode) {{
                    // Head juggling: bounce on top of head (head top at y=1.95)
                    // Ball center at y = 2.15 + 0.2*sin(...) => ranges 1.95 to 2.35
                    // Bottom of ball touches head top at lowest point (1.95)
                    const bounceHeight = 2.15 + Math.sin(state.walkCycle * 2) * 0.2;
